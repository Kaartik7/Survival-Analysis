...
fit.weib4 <- survreg( Surv(Time, Relapse) ~ Treatment , data=data_1, 
                      dist="weibull")

fit.log4 <- survreg( Surv(Time, Relapse) ~ Treatment , data=data_1, 
                     dist="lognormal")

fit.logl <- survreg( Surv(Time, Relapse) ~ Treatment , data=data_1, 
                     dist="loglogistic")
...
```{r}
# Kaplan-Meier survival curve for lognormal model
km_fit <- survfit(Surv(Time, Relapse) ~ Treatment, data = data_1)
